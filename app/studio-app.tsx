...
type PlanVersion = { id: string; label: string; summary: string; createdAt: string; source: "initial" | "revision" | "regenerate"; project: Project; previewImageUrl?: string; previewVersions?: string[] };
...
const ROOF_OPTIONS: { value: RoofTemplate; label: string }[] = [
...
export default function StudioApp() {
  ...
  const [roofTemplate, setRoofTemplate] = useState<RoofTemplate>("none");
  ...
  const activeVersion = useMemo(() => versionHistory.find(entry => entry.id === activeVersionId) ?? versionHistory[versionHistory.length - 1], [activeVersionId, versionHistory]);
  ...
  const stackedMultiPlans = useMemo(() => Array.from({ length: floorCount }, (_, index) => index + 1).map((floor, index) => {
    ...
    return {
      ...
      level: index,
      elevation: index * FLOOR_STACK_HEIGHT,
  ...
  function generate() {
    ...
    setVersionHistory(nextHistory); setActiveVersionId(entry.id); setProject(entry.project); setSelectedId(""); setBlueprintImage(""); setViewMode("2d"); setCameraMode("orbit"); setStage("plan");
    void saveProjectDesignSnapshot(entry.project, nextHistory, entry.id);
  }
  function loadVersion(entry: PlanVersion) {
    setBrief(entry.project.brief); setProject(entry.project); setActiveVersionId(entry.id); setSelectedId(""); setBlueprintImage(""); setRevisionText("");
  ...
  function regenerateVersion() {
    ...
    setVersionHistory(nextHistory); setActiveVersionId(entry.id); setProject(entry.project); setSelectedId(""); setBlueprintImage(""); setViewMode("2d"); setCameraMode("orbit");
    void saveProjectDesignSnapshot(entry.project, nextHistory, entry.id);
  }
  ...
  function handlePreviewGenerated(url: string) {
    if (!activeVersionId) return;
    setVersionHistory(current => current.map(entry => {
      if (entry.id !== activeVersionId) return entry;
      const currentVersions = entry.previewVersions ?? [];
      const nextVersions = currentVersions[currentVersions.length - 1] === url ? currentVersions : [...currentVersions, url];
...
const roofControls = viewMode === "3d" ? <div className="roof-style-picker" aria-label="Roof style"><span>Roof</span>{ROOF_OPTIONS.map(option => <button key={option.value} className={roofTemplate===option.value?"active":""} onClick={()=>setRoofTemplate(option.value)}>{option.label}</button>)}</div> : null;
  async function applyRevision() {
    ...
    try {
      ...
      const nextHistory = [...versionHistory, entry];
      setBrief(data.brief); setProject(entry.project); setVersionHistory(nextHistory); setActiveVersionId(entry.id); setSelectedId(""); setBlueprintImage(""); setRevisionText("");
  ...
  function renderEngineTrace(trace: Project["generationTrace"] | undefined) {
    if (!trace) return null;
    return <details className="engine-trace-card compact"><summary><span><Ruler size={13}/> Engine trace</span><small>{trace.selectedLabel}</small></summary><p>Selected: <b>{trace.selectedLabel}</b></p>{trace.candidates.map(candidate=><article key={candidate.label} className={candidate.selected?"selected":candidate.valid?"valid":"error"}><div><b>{candidate.label}</b><small>{candidate.source} / score {candidate.score}</small></div><em>{candidate.selected?"WIN":candidate.valid?"OK":"FAIL"}</em>{candidate.errors.map(error=><span key={error}>{error}</span>)}{!candidate.errors.length&&candidate.warnings.slice(0,2).map(warning=><span key={warning}>{warning}</span>)}</article>)}</details>;
  }
  function renderSingleSidebar() {
    if (!project) return null;
    return <><section className="generated-summary"><span className="eyebrow">Generated plan</span><h2>{project.brief.title}</h2><ul className="plan-facts"><li><b>Plot</b><span>{project.brief.plotWidth} x {project.brief.plotDepth} {project.brief.unit}</span></li><li><b>Facing</b><span>{project.brief.facing}</span></li><li><b>Road side</b><span>{project.brief.roadSide}</span></li><li><b>Quality</b><span>{architecture?`${architecture.score}/100`:"Checked"}</span></li></ul><div className="validation-card compact"><div className={errors.length?"status error":"status valid"}>{errors.length?<AlertTriangle/>:<Check/>}<span><b>{errors.length?"Needs correction":"Architecture checked"}</b><small>{errors.length?`${errors.length} blocking issue(s)`:"No blocking geometry issues"}</small></span></div>{errors.map(e=><p key={e}>{e}</p>)}{!errors.length&&architecture?.warnings.map(w=><p className="arch-warning" key={w}>{w}</p>)}</div>{(designSaving||designSaveError)&&<div className={designSaveError?"revision-feedback error":"revision-feedback"}><b>{designSaveError?"Project save failed":"Saving project design..."}</b>{designSaveError&&<span>{designSaveError}</span>}</div>}</section>{versionHistory.length>0&&<div className="version-card"><h3><History size={13}/> Versions</h3>{versionHistory.map(entry=><button key={entry.id} className={activeVersionId===entry.id?"active":""} onClick={()=>loadVersion(entry)}><span><b>{entry.label}</b><small>{entry.summary}</small></span><em>{entry.createdAt}</em></button>)}</div>}<div className="revision-card"><h3><Sparkles size={13}/> Revise this plan</h3><textarea value={revisionText} onChange={e=>setRevisionText(e.target.value)} rows={4} placeholder="Example: move kitchen closer to dining, make hallway wider, add porch near entry."/><button className="primary-btn wide" onClick={applyRevision} disabled={revisionLoading||revisionText.trim().length<5}>{revisionLoading?"Applying safely...":<>Apply revision <ChevronRight size={15}/></>}</button>{revisionFeedback.summary&&<div className={revisionFeedback.kind==="error"?"revision-feedback error":"revision-feedback"}><b>{revisionFeedback.summary}</b>{revisionFeedback.details.map(item=><span key={item}>{item}</span>)}</div>}</div><div className="room-key"><h3>Room schedule</h3>{project.plans[0].rooms.map(r=><button key={r.id} className={selectedId===r.id?"active":""} onClick={()=>setSelectedId(r.id)}><i style={{background:r.color}}/><span>{r.name}</span><small>{r.width.toFixed(1)} x {r.depth.toFixed(1)}</small></button>)}</div>{renderEngineTrace(project.generationTrace)}</>;
  }
  function renderMultiSidebar() {
    if (!activeMultiProject) return null;
    return <><section className="generated-summary"><span className="eyebrow">Selected floor</span><h2>Floor {activeMultiFloor}</h2><ul className="plan-facts"><li><b>Plot</b><span>{activeMultiProject.brief.plotWidth} x {activeMultiProject.brief.plotDepth} {activeMultiProject.brief.unit}</span></li><li><b>Facing</b><span>{activeMultiProject.brief.facing}</span></li><li><b>Road side</b><span>{activeMultiProject.brief.roadSide}</span></li><li><b>Quality</b><span>{activeMultiArchitecture?`${activeMultiArchitecture.score}/100`:"Checked"}</span></li></ul><div className="validation-card compact"><div className={activeMultiErrors.length?"status error":"status valid"}>{activeMultiErrors.length?<AlertTriangle/>:<Check/>}<span><b>{activeMultiErrors.length?"Needs correction":"Architecture checked"}</b><small>{activeMultiErrors.length?`${activeMultiErrors.length} issue(s)`:"No blocking geometry issues"}</small></span></div>{activeMultiErrors.map(error=><p key={error}>{error}</p>)}{!activeMultiErrors.length&&activeMultiArchitecture?.warnings.map(warning=><p className="arch-warning" key={warning}>{warning}</p>)}</div></section>{activeMultiVersionHistory.length>0&&<div className="version-card"><h3><History size={13}/> Versions</h3>{activeMultiVersionHistory.map(entry=><button key={entry.id} className={multiFloorActiveVersionId[activeMultiFloor]===entry.id?"active":""} onClick={()=>loadMultiFloorVersion(entry)}><span><b>{entry.label}</b><small>{entry.summary}</small></span><em>{entry.createdAt}</em></button>)}</div>}<div className="revision-card"><h3><Sparkles size={13}/> Revise floor {activeMultiFloor}</h3><textarea value={activeMultiRevisionText} onChange={e=>setMultiFloorRevisionText(current=>({...current,[activeMultiFloor]:e.target.value}))} rows={4} placeholder="Example: move bedroom closer to hall, widen stair landing, add balcony near front."/><button className="primary-btn wide" onClick={applyMultiFloorRevision} disabled={multiFloorRevisionLoading||activeMultiRevisionText.trim().length<5}>{multiFloorRevisionLoading?"Applying safely...":<>Apply revision <ChevronRight size={15}/></>}</button>{activeMultiRevisionFeedback.summary&&<div className={activeMultiRevisionFeedback.kind==="error"?"revision-feedback error":"revision-feedback"}><b>{activeMultiRevisionFeedback.summary}</b>{activeMultiRevisionFeedback.details.map(item=><span key={item}>{item}</span>)}</div>}</div><div className="room-key"><h3>Room schedule</h3>{activeMultiProject.plans[0].rooms.map(room=><button key={room.id} className={selectedId===room.id?"active":""} onClick={()=>setSelectedId(room.id)}><i style={{background:room.color}}/><span>{room.name}</span><small>{room.width.toFixed(1)} x {room.depth.toFixed(1)}</small></button>)}</div>{renderEngineTrace(activeMultiProject.generationTrace)}</>;
  }
  ...
  return <main className="two-d-app">
    ...
    {(stage==="plan"||stage==="multiSetup")&&viewMode==="3d"&&roofControls}
    ...
    {stage==="project"&&<section className="project-dashboard">
      ...
      {projectsLoading&&<div className="project-loading"><span className="draft-loader"/>Syncing projects...</div>}
