...
export default function StudioApp({ initialStage = "login" }: { initialStage?: "login" | "prompt" }) {
  ...
  const [viewMode, setViewMode] = useState<"2d" | "2d_preview" | "3d">("2d");
  ...
  const activeVersion = useMemo(() => versionHistory.find(v => v.id === activeVersionId), [versionHistory, activeVersionId]);
  const hasPreview = useMemo(() => activeVersion?.previewVersions && activeVersion.previewVersions.length > 0, [activeVersion]);
  const handlePreviewGenerated = (url: string) => {
    const nextHistory = versionHistory.map(v => {
      if (v.id === activeVersionId) {
        const currentVersions = v.previewVersions || [];
        const nextVersions = [...currentVersions, url];
        return { ...v, previewImageUrl: url, previewVersions: nextVersions };
      }
      return v;
    });
    setVersionHistory(nextHistory);
    if (project) {
      void saveProjectDesignSnapshot(project, nextHistory, activeVersionId);
    }
  };
  ...
  const [stage, setStage] = useState<"login"|"project"|"prompt"|"review"|"plan">(initialStage); const [loading, setLoading] = useState(false); const [apiError, setApiError] = useState("");
