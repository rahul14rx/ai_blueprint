...
function cleanJsonText(text: string) {
  let new_ = text.trim().replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/i, "");
  return new_;
}
        ...
        contents: [{
          ...
          parts: [{
            text: [
              ...
              "For furnitureRequirements, return standard items needed per requested room (e.g., bed and wardrobe for bedrooms, sofa and TV unit for living room). Provide width and depth in feet.",
