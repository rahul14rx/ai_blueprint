...
function insideDoorGap(gaps: OpeningGap[], orientation: Edge["orientation"], coord: number, start: number, end: number) {
  const mid = (start + end) / 2;
  return gaps.some(gap => gap.orientation === orientation && sameCoord(gap.coord, coord) && gap.start - EPSILON <= mid && gap.end + EPSILON >= mid);
}
...
function mergeSegments(segments: WallSegment3D[]) {
  ...
  for (const segment of segments) {
    ...
    if (previous && previous.kind === segment.kind && previous.orientation === segment.orientation && previous.thickness === segment.thickness) {
...
export function buildPlan3DGeometry(plan: FloorPlan): { walls: WallSegment3D[] } {
  ...
  for (const [key, groupEdges] of groups) {
    ...
    doorGaps.filter(gap => gap.orientation === orientation && sameCoord(gap.coord, coord)).forEach(gap => {
    ...
    for (let index = 0; index < points.length - 1; index += 1) {
      ...
      if (end - start < EPSILON || insideDoorGap(doorGaps, orientation, coord, start, end)) continue;
      ...
      const segment: WallSegment3D = orientation === "horizontal"
        ? {
          id: `wall-${orientation}-${roundKey(coord)}-${roundKey(start)}-${roundKey(end)}`,
          kind,
          orientation,
          x1: start,
          z1: coord,
